...
; 下面在C盘根目录中寻找 *.BIN
LABEL_REPEAT:			; 装载文件循环，循环变量为count = 0~1
...
LABEL_FILENAME_FOUND:	; 找到 LOADER.BIN 后便来到这里继续
	...
	shl cx, 3			; 簇号*BPB_SecPerClus[=8] = 文件的相对起始扇区号
...
LABEL_GOON_LOADING_FILE:
	...
	shl ax, 3				; 簇号*BPB_SecPerClus[=8] = 簇的相对起始扇区号
	add	ax, dx
...
LABEL_FILE_LOADED:
	...
	cmp byte [count], 2
	jnz LABEL_REPEAT
	mov dh, 3		; "Load finish"
	call DispStr		; 显示字符串

; **********************************************************************
	jmp	3000h:OffsetOfFile	; 这一句正式跳转到已加载到内
						; 存中的 CMD.BIN 的开始处，
						; 开始执行 CMD.BIN 的代码。
						; LOADER.BIN 的使命到此结束
; **********************************************************************
...
BS_DrvNum		DB 0x80	; 中断 13 的驱动器号（硬盘）
wSectorNo		dw	0	; 要读取的扇区数
bOdd			db	0	; 奇数还是偶数
count			db	0	; 已装入的文件数（循环变量）
BaseOfFile: 	dw 1000h, 3000h ; 内核与命令行模块的内存基地址
; 文件名字符串
FileName:		db	"KERNEL  BIN" ; KERNEL.BIN文件名
FileName1:		db	"CMD     BIN" ; CMD.BIN文件名
; 为简化代码，下面每个字符串的长度均为MessageLength（=11），似串数组
MessageLength	equ	11
BootMessage:		db	"Load Kernel" 	; 11字节，不够则用空格补齐。序号0
Message1			db	"LoadCommand" 	; 11字节，不够则用空格补齐。序号1
Message2			db	"Not found! "	; 11字节，不够则用空格补齐。序号2
Message3			db	"Load finish"	; 11字节，不够则用空格补齐。序号3
;==============================================================
...
ReadSector:
	...
	mov	cl, ah		; CL <- 起始扇区号
	mov	dh, al		; DH <- y
	shr	al, 2			; y >> 1 （等价于y/BPB_NumHeads，软盘有2个磁头）
	mov	ch, al		; CH <- 柱面号
	and	dh, 3		; DH & 1 = 磁头号
	pop	bx			; 恢复BX
