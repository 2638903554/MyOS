...
SetTimer:
	...
	mov ax,1193182/500 	; 每秒100次中断（10ms一次）
...
[SECTION .data]			; 数据段在此
	...
	MainStr	db 'Kernel begin ...'	,0Dh,0Ah ; 内核启动时显示的字符串+回车和换行
...
; ---------------------------------------------------------------
CharPos dw 80*6*2 ; 字符位置，初始为第6行（5号行）开始处
DispVal: ; 显示16位整数值串（以AX为传递参数）
	...
	mov bx,[CharPos]
	mov [gs:bx],ax		; 显示字符：[gs : [CharPos]] = AX
...
ShowChar: ; 显示一个十六进制数字符：0~9、A~F（以AL为传递参数）
	...
	ret
; ---------------------------------------------------------------	
;下面的函数显示当前PCB中16个寄存器的值
; ---------------------------------------------------------------	
; 显示当前PCB中所有寄存器的值函数
DispPCB:
	mov di,[p_proc_ready]	; DI = 当前PCB起始地址
	mov cx,16			; 显示16个寄存器的值
	again:				; 循环
	mov ax,[di]			; AX = PCB中当前的寄存器值
	call DispVal		; 调用显示十六进制数值的函数
	add di,2			; DI = PCB中下一寄存器的偏移量
	dec cx				; 循环变量减1
	jnz again			; > 0 继续循环
	ret
